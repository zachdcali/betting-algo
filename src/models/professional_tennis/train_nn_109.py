...
# Load the existing ML-ready dataset (leak-free)
...
# Remove low-importance features with SYMMETRIC removal (Grok's aggressive approach)
# This fixes the bias issue by ensuring P1/P2 feature parity
...
removed_count = original_feature_count - len(feature_cols)
print(f"   Features for training: {len(feature_cols)} (removed {removed_count} low-importance features)")
...
# Calculate additional calibration metrics for betting
...
# Save the symmetric feature model with descriptive name
print(f"\nSaving symmetric Neural Network model...")
symmetric_model_path = os.path.join(output_dir, 'neural_network_symmetric_features.pth')
symmetric_scaler_path = os.path.join(output_dir, 'scaler_symmetric_features.pkl')

torch.save(model.state_dict(), symmetric_model_path)
with open(symmetric_scaler_path, 'wb') as f:
    pickle.dump(scaler, f)

print(f"   Symmetric Neural Network model saved to: {symmetric_model_path}")
print(f"   Symmetric scaler saved to: {symmetric_scaler_path}")
print(f"   Features used: {len(feature_cols)} (symmetric feature removal)")
print(f"   This fixes P1/P2 bias issues from asymmetric feature removal")