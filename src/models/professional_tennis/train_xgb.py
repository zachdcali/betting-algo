...
plt.title('Professional Tennis XGBoost - Top 20 Feature Importances')
...
plt.ylabel('Density')
plt.title('Professional Tennis XGBoost - Prediction Probability Distribution')
plt.legend()
plt.tight_layout()
...
plt.title('Professional Tennis XGBoost - Monthly Accuracy Over Time')
plt.xlabel('Month')
plt.ylabel('Accuracy')
plt.legend()
...
if hasattr(xgb_model, 'evals_result_'):
    ...
    if 'validation_0' in results:
        ...
        plt.title('Professional Tennis XGBoost - Training Curve')
...
if os.path.exists(old_model_path):
    try:
        ...
        print(f"  (old xgboost_model.json comparison skipped: {e})")
else:
