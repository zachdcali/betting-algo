...
async def retry_with_backoff(func, max_wait=300, backoff_multiplier=1.5):
    """
    Retry a function with exponential backoff until it succeeds.
    Will keep trying indefinitely but with increasing delays (capped at max_wait).
    """
    attempt = 1
...
class UTRScraper:
    def __init__(self, email, password, headless=True, data_dir=None, max_processes=8):
        self.email = email
    ...
    def update_player_mapping(self, player_id, player_name, current_utr=None):
        """Update master player mapping CSV"""
        mapping_file = self.data_dir / "player_mapping.csv"
        
        # Create header if file doesn't exist
        if not mapping_file.exists():
            with open(mapping_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['utr_id', 'primary_name', 'name_variants', 'current_utr', 'country', 'last_updated'])
        ...
        if player_name:
            ...
            if len(parts) >= 2:
                # Add variants like "Last F.", "F. Last", "LAST F."
        ...
        if mapping_file.exists():
            try:
                with open(mapping_file, 'r') as f:
                    ...
                    existing_data = [row for row in reader if row['utr_id'] != player_id]
            except:
        ...
        new_row = {
            'utr_id': player_id,
            'primary_name': player_name or '',
            'name_variants': '|'.join(name_variants) if name_variants else '',
            'current_utr': current_utr or '',
            'country': '',  # Can be filled later if available
    ...
    def update_tournament_mapping(self, tournament_name, location=None, country=None):
        """Update tournament location mapping for future weather data"""
        mapping_file = self.data_dir / "tournament_locations.csv"
        
        # Create header if file doesn't exist
        if not mapping_file.exists():
            with open(mapping_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['tournament_name', 'location', 'country', 'latitude', 'longitude', 'last_updated'])
        ...
        if mapping_file.exists():
            try:
                with open(mapping_file, 'r') as f:
                    reader = csv.DictReader(f)
                    existing_tournaments = {row['tournament_name'] for row in reader}
            except:
                pass
        
        # Only add if new tournament
        if tournament_name and tournament_name not in existing_tournaments:
            with open(mapping_file, 'a', newline='') as f:
                ...
                writer.writerow([
                    ...
                    location or '',
                    country or '',
                    '',  # latitude - to be filled later
                    '',  # longitude - to be filled later
    ...
    async def login(self):
        """Log in to UTR website with unlimited retries"""
        if self.logged_in:
            return True
            
        async def _login_attempt():
            logger.info("Logging in to UTR...")
            await self.page.goto("https://app.utrsports.net/login", wait_until="load", timeout=60000)
            ...
            await self.page.wait_for_selector('#emailInput', state="visible", timeout=60000)
        ...
        # Use retry wrapper for unlimited login attempts
        return await retry_with_backoff(_login_attempt)
    ...
    async def get_player_profile(self, player_id):
        ...
        try:
            ...
            async def _navigate_to_profile():
                await self.page.goto(profile_url, wait_until="load", timeout=60000)
            
            # Use retry wrapper for unlimited navigation attempts
            await retry_with_backoff(_navigate_to_profile)
            ...
            try:
                    ...
                    for utr_elem in utr_elems:
                        ...
                        if utr_value and "." in utr_value:  # Make sure it looks like a UTR
                            try:
                                ...
                                # Validate UTR range (1.0-16.5)
                                if 1.0 <= current_utr <= 16.5:
                                    logger.info(f"Found valid UTR value: {current_utr}")
                                    break
                                else:
                                    logger.warning(f"UTR value {current_utr} outside valid range (1.0-16.5)")
                                    current_utr = None
                ...
                # Add to master player mapping CSV
                self.update_player_mapping(player_id, player_name, profile_data.get('current_utr'))
    ...
    async def get_player_match_history(self, player_id, year=None, limit=200):
        ...
        try:
            ...
            async def _navigate_to_matches():
                await self.page.goto(profile_url, wait_until="load", timeout=60000)
            
            # Use retry wrapper for unlimited navigation attempts
            await retry_with_backoff(_navigate_to_matches)
            ...
            for match_idx, card in enumerate(match_cards):
                try:
                    ...
                    # Update tournament mapping for location data
                    self.update_tournament_mapping(event_name)
    ...
    async def get_top_players(self, gender="men", limit=100):
        """
        Get top players from UTR rankings page
        ...
        Returns:
        DataFrame: Top players with rank, name, id, and utr
        """
        logger.info(f"Getting top {limit} {gender}'s players from UTR rankings")
        
        try:
            # Navigate to rankings page
            if gender.lower() == "men":
                rankings_url = "https://www.utrsports.net/pages/utr-men-tennis-rankings"
            else:
                rankings_url = "https://www.utrsports.net/pages/utr-women-tennis-rankings"
            
            await self.page.goto(rankings_url, timeout=30000)
            await asyncio.sleep(3)
            
            # Wait for page to load - be more flexible with selectors
            try:
                await self.page.wait_for_selector('body', timeout=10000)
                await asyncio.sleep(5)  # Let content load
            except:
                pass
            
            players = []
            
            # Look for player links - be very broad in search
            player_links = await self.page.query_selector_all('a[href*="/profiles/"]')
            
            for idx, link in enumerate(player_links[:limit*2]):  # Check more links in case some aren't players
                ...
                try:
                    ...
                    # Skip navigation links (not player names)
                    skip_terms = ['view', 'profile', 'more', 'show', 'rankings', 'leaderboard', 'home', 'news']
                    if any(term in player_name.lower() for term in skip_terms):
                        continue
                    ...
                    # Skip if we already have this player (duplicate links)
                    if any(p['id'] == player_id for p in players):
                        continue
                    
                    # Look for UTR rating near the link (in parent or sibling elements)
                    utr_rating = None
                    try:
                        # Check parent element for UTR
                        parent = await link.query_selector('xpath=..')
                        if parent:
                            parent_text = await parent.inner_text()
                            # Look for UTR pattern (like 15.42, 8.5, etc.)
                            utr_match = re.search(r'\b(1[0-6]\.\d+|[1-9]\.\d+)\b', parent_text)
                            if utr_match:
                                potential_utr = float(utr_match.group(1))
                                if 1.0 <= potential_utr <= 16.5:
                                    utr_rating = potential_utr
                    except:
                        pass
                    ...
                    players.append({
                        "rank": rank,
                        "name": player_name,  # Full name from link text
                        "id": player_id,
                        "utr": utr_rating
                    })
                    
                    logger.info(f"Found player {rank}: {player_name} (ID: {player_id}, UTR: {utr_rating})")
                    
                except Exception as e:
                    logger.warning(f"Error processing player link {idx}: {e}")
                    continue
            
            if not players:
                logger.error("No players found on rankings page")
                return pd.DataFrame()
            
            df = pd.DataFrame(players)
            
            # Save rankings to CSV for finish_processing.py
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            rankings_file = self.data_dir / f"{gender}_utr_rankings_{timestamp}.csv"
            df.to_csv(rankings_file, index=False)
            logger.info(f"Saved {len(players)} {gender}'s players to {rankings_file}")
    ...
    async def collect_top_players_data_sequential(self, gender="men", limit=100, player_limit=None, include_matches=True, include_ratings=True, years=None, utr_threshold=1.0, dynamic_years=False):
        """
    ...
    async def collect_top_players_data_concurrent(self, gender="men", limit=100, player_limit=None, include_matches=True, include_ratings=True, years=None, utr_threshold=1.0, dynamic_years=False):
        """
        Advanced concurrent version using dynamic work pools and staged processing
        
        Stage 1: Top players (parallel)
        Stage 2: 1st degree opponents (dynamic work pool)  
        Stage 3: 2nd degree opponents (dynamic work pool)
        """
        if years is None:
            years = ["2025", "2024", "2023", "2022"]
        ...
        logger.info(f"🚀 Starting advanced concurrent collection for {len(top_players)} players with {self.max_processes} processes")
        
        # STAGE 1: Process top players in parallel with dynamic work pool
        logger.info("📊 STAGE 1: Processing top 100 players...")
        top_player_work_queue = asyncio.Queue()
        
        # Add all top players to work queue
        for _, player in top_players.iterrows():
            await top_player_work_queue.put({
                'player_id': player['id'],
                'name': player['name'],
                'stage': 'top_players'
            })
        
        # Create worker tasks for stage 1
        stage1_tasks = []
        all_opponents_collected = asyncio.Queue()  # Collect opponents from all workers
        
        for worker_id in range(self.max_processes):
            task = asyncio.create_task(
                self._dynamic_worker(
                    worker_id, top_player_work_queue, all_opponents_collected,
                    include_matches, include_ratings, years, utr_threshold, dynamic_years
                )
            )
            stage1_tasks.append(task)
        
        # Wait for all top players to be processed
        await asyncio.gather(*stage1_tasks)
        logger.info("✅ STAGE 1 completed: All top players processed")
        
        # STAGE 2: Extract unique 1st degree opponents and process them
        logger.info("📊 STAGE 2: Processing 1st degree opponents...")
        unique_first_degree = await self._extract_unique_opponents(all_opponents_collected, existing_ids=set(top_players['id']))
        
        if unique_first_degree:
            logger.info(f"Found {len(unique_first_degree)} unique 1st degree opponents")
            
            # Create work queue for 1st degree opponents
            first_degree_queue = asyncio.Queue()
            second_degree_collected = asyncio.Queue()
            
            for opponent_id in unique_first_degree:
                await first_degree_queue.put({
                    'player_id': opponent_id,
                    'name': f"Opponent_{opponent_id}",
                    'stage': '1st_degree'
                })
            
            # Create worker tasks for stage 2
            stage2_tasks = []
            for worker_id in range(self.max_processes):
                task = asyncio.create_task(
                    self._dynamic_worker(
                        worker_id, first_degree_queue, second_degree_collected,
                        include_matches, include_ratings, years, utr_threshold, dynamic_years
                    )
                )
            ...
            logger.info("✅ STAGE 2 completed: All 1st degree opponents processed")
            
            # STAGE 3: Extract unique 2nd degree opponents and process them
            logger.info("📊 STAGE 3: Processing 2nd degree opponents...")
            all_existing_ids = set(top_players['id']) | unique_first_degree
            unique_second_degree = await self._extract_unique_opponents(second_degree_collected, existing_ids=all_existing_ids)
            
            if unique_second_degree:
                logger.info(f"Found {len(unique_second_degree)} unique 2nd degree opponents")
                
                # Create work queue for 2nd degree opponents
                second_degree_queue = asyncio.Queue()
                
                for opponent_id in unique_second_degree:
                    await second_degree_queue.put({
                        'player_id': opponent_id,
                        'name': f"2ndDegree_{opponent_id}",
                        'stage': '2nd_degree'
                    })
                
                # Create worker tasks for stage 3
                stage3_tasks = []
                dummy_queue = asyncio.Queue()  # Not collecting further opponents
                
                for worker_id in range(self.max_processes):
                    task = asyncio.create_task(
                        self._dynamic_worker(
                            worker_id, second_degree_queue, dummy_queue,
                            include_matches, include_ratings, years, utr_threshold, dynamic_years
                        )
                    )
                ...
                logger.info("✅ STAGE 3 completed: All 2nd degree opponents processed")
        
        logger.info("🎉 Advanced concurrent collection completed! All stages finished.")
        return top_players

    async def _dynamic_worker(self, worker_id, work_queue, opponent_collection_queue, include_matches, include_ratings, years, utr_threshold, dynamic_years):
        """
        Dynamic worker that processes tasks from work queue until empty
        Each worker has its own browser instance and grabs work as available
        """
        # Create a separate data directory for this worker
        worker_data_dir = self.data_dir / f"worker_{worker_id}"
        
        # Create new scraper instance for this worker
        scraper = UTRScraper(
            email=self.email,
            password=self.password,
            headless=self.headless,
            data_dir=worker_data_dir,
            max_processes=1  # Each worker is single-process
        )
        
        processed_count = 0
        logger.info(f"🎾 Worker {worker_id}: Starting dynamic processing")
        
        try:
            async with scraper:
                if not await scraper.login():
                    logger.error(f"❌ Worker {worker_id}: Failed to login")
                    return
                
                # Keep processing until work queue is empty
                while True:
                    try:
                        # Get work item with timeout (exit if no work available)
                        work_item = await asyncio.wait_for(work_queue.get(), timeout=2.0)
                        
                        player_id = work_item['player_id']
                        name = work_item['name']
                        stage = work_item['stage']
                        
                        processed_count += 1
                        logger.info(f"📊 Worker {worker_id}: Processing {stage} - {name} (#{processed_count})")
                        
                        # Collect all opponents from this player's matches
                        collected_opponents = set()
                        
                        try:
                            # Collect ratings if requested
                            if include_ratings:
                                await scraper.get_player_ratings(player_id, name)
                            
                            # Collect matches if requested and extract opponents
                            if include_matches:
                                for year in years:
                                    try:
                                        matches = await scraper.get_player_matches(player_id, name, year)
                                        if matches is not None and not matches.empty:
                                            # Extract opponent IDs and high UTR opponents
                                            opponent_ids = matches['opponent_id'].dropna().unique()
                                            collected_opponents.update(opponent_ids)
                                            
                                            # Filter high UTR opponents for next stage
                                            high_utr_matches = matches[
                                                pd.to_numeric(matches['opponent_utr_displayed'], errors='coerce') >= utr_threshold
                                            ]
                                            high_utr_ids = high_utr_matches['opponent_id'].dropna().unique()
                                            
                                            # Send opponents to collection queue for next stage
                                            for opp_id in high_utr_ids:
                                                await opponent_collection_queue.put(opp_id)
                                        
                                        await asyncio.sleep(random.uniform(1.0, 2.0))
                                    except Exception as e:
                                        logger.error(f"Worker {worker_id}: Error getting matches for {name} in {year}: {e}")
                            
                            # Add delay between players
                            await asyncio.sleep(random.uniform(1.0, 2.0))
                            
                        except Exception as e:
                            logger.error(f"Worker {worker_id}: Error processing {name}: {e}")
                        
                        # Mark task as done
                        work_queue.task_done()
                        
                    except asyncio.TimeoutError:
                        # No more work available, exit worker
                        logger.info(f"✅ Worker {worker_id}: No more work, completed {processed_count} items")
                        break
                    except Exception as e:
            ...
            # Merge worker data back to main directory
            await self._merge_worker_data(worker_id, worker_data_dir)

    async def _extract_unique_opponents(self, opponent_queue, existing_ids):
        """
        Extract unique opponent IDs from collection queue, excluding already processed IDs
        """
        unique_opponents = set()
        
        # Collect all opponents from the queue
        while not opponent_queue.empty():
            try:
                opponent_id = await asyncio.wait_for(opponent_queue.get(), timeout=0.1)
                if opponent_id not in existing_ids:
                    unique_opponents.add(opponent_id)
                opponent_queue.task_done()
            except asyncio.TimeoutError:
                break
            except Exception as e:
                logger.error(f"Error extracting opponent ID: {e}")
        
        return unique_opponents

    async def _merge_worker_data(self, worker_id, worker_data_dir):
        """
        Merge data from worker-specific directory back to main directory
        """
        try:
            import shutil
            
            worker_data_path = Path(worker_data_dir)
            if not worker_data_path.exists():
                return
            
            logger.info(f"📁 Worker {worker_id}: Merging data to main directory")
            
            # Merge each subdirectory
            for subdir in ['players', 'matches', 'ratings']:
                src_dir = worker_data_path / subdir
                dst_dir = self.data_dir / subdir
                
                if src_dir.exists():
                    dst_dir.mkdir(parents=True, exist_ok=True)
                    
                    # Copy all files from worker directory to main directory
            ...
            logger.info(f"✅ Worker {worker_id}: Data merged and cleaned up")
            
        except Exception as e:
            logger.error(f"❌ Worker {worker_id}: Error merging data: {e}")
    ...
    async def collect_top_players_data(self, gender="men", limit=100, player_limit=None, include_matches=True, include_ratings=True, years=None, utr_threshold=1.0, dynamic_years=False):
        """
        Main collection method - uses concurrent processing if max_processes > 1
        """
        if self.max_processes > 1:
            logger.info(f"🚀 Using concurrent processing with {self.max_processes} processes")
            return await self.collect_top_players_data_concurrent(
                gender, limit, player_limit, include_matches, 
                include_ratings, years, utr_threshold, dynamic_years
            )
        else:
            logger.info("📝 Using sequential processing (single process)")
            return await self.collect_top_players_data_sequential(
                gender, limit, player_limit, include_matches, 
                include_ratings, years, utr_threshold, dynamic_years
            )
...
if __name__ == "__main__":
    ...
    async def main():
        async with UTRScraper(email=email, password=password, headless=True) as scraper:
            ...
            logger.info("Starting full top-100 ATP data collection")
            
            # Collect comprehensive data for top 100 men's players
            await scraper.collect_top_players_data(
                gender="men", 
                limit=100,  # Top 100 ATP players
                include_matches=True,
                include_ratings=True,
                years=["2025", "2024", "2023", "2022", "2021", "2020"],
                utr_threshold=1.0,  # Include all players above 1.0 UTR
                dynamic_years=True   # Use intelligent year selection
            )
            logger.info("Full top-100 data collection completed successfully!")
