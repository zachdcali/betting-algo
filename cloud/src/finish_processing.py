...
def process_opponent_worker(args):
    ...
    """
    opponent_id, email, password, data_dir_str, years, dynamic_years, delay = args
    data_dir = Path(data_dir_str)  # Convert string back to Path object

    async def inner():
        # Stagger login attempts to avoid overwhelming UTR servers
        ...
        try:
            ...
            # Retry login up to 3 times with increasing delays
            login_success = False
            for attempt in range(3):
                login_success = await scraper.login()
                if login_success:
                    break
                logger.warning(f"Process ID {os.getpid()}: Login attempt {attempt + 1} failed for opponent {opponent_id}, retrying in {(attempt + 1) * 10} seconds...")
                await asyncio.sleep((attempt + 1) * 10)
                
            if not login_success:
                logger.error(f"Process ID {os.getpid()}: Failed to login after 3 attempts for opponent {opponent_id}")
                return None, None
...
def process_second_degree_opponent_worker(args):
    ...
    opponent_id, email, password, data_dir_str, delay = args
    ...
    async def inner():
        ...
        try:
            ...
            for attempt in range(3):
                ...
                if login_success:
                    break
                logger.warning(f"Process ID {os.getpid()}: Login attempt {attempt + 1} failed for second-degree opponent {opponent_id}, retrying in {(attempt + 1) * 10} seconds...")
                await asyncio.sleep((attempt + 1) * 10)
                
            if not login_success:
                logger.error(f"Process ID {os.getpid()}: Failed to login after 3 attempts for second-degree opponent {opponent_id}")
                return
...
def process_top_player_worker(args):
    ...
    player_id, email, password, data_dir_str, years, dynamic_years, delay = args
    data_dir = Path(data_dir_str)
    ...
    async def inner():
        ...
        try:
            ...
            for attempt in range(3):
                ...
                if login_success:
                    break
                logger.warning(f"Process ID {os.getpid()}: Login attempt {attempt + 1} failed for player {player_id}, retrying in {(attempt + 1) * 10} seconds...")
                await asyncio.sleep((attempt + 1) * 10)  # 10, 20, 30 second delays
                
            if not login_success:
                logger.error(f"Process ID {os.getpid()}: Failed to login after 3 attempts for top player {player_id}")
                return None, None
...
async def collect_top_players_data(data_dir, email, password, years=None, dynamic_years=True):
    ...
    if not top_player_ids:
        print("No existing ranking file found. Scraping top 100 players from UTR...")
        
        # Create scraper instance to get fresh rankings
        scraper = UTRScraper(email=email, password=password, headless=True, data_dir=data_dir)
        ...
        try:
            ...
            login_success = await scraper.login()
            if not login_success:
                print("Failed to login to UTR. Cannot proceed.")
                return False, set()
            
            # Get top 100 players (this creates the CSV file)
            players_df = await scraper.get_top_players(gender="men", limit=100)
            
            if players_df is not None and not players_df.empty:
                top_player_ids = set(players_df['id'].dropna().astype(str))
                print(f"Successfully scraped {len(top_player_ids)} top players from UTR")
            else:
                print("Failed to scrape top players from UTR")
                return False, set()
                
        except Exception as e:
            print(f"Error scraping top players: {e}")
            return False, set()
        finally:
            await scraper.close_browser()
    ...
    with Pool(processes=num_processes) as pool:
        worker_args = [(pid, email, password, str(data_dir), years, dynamic_years, i * 5.0) for i, pid in enumerate(top_player_ids)]
        for result in pool.imap_unordered(process_top_player_worker, worker_args):
            if result is not None:
                second_ids, processed_id = result
...
def collect_missing_opponent_data(data_dir, email, password, years=None, dynamic_years=True):
    ...
    with Pool(processes=num_processes) as pool:
        worker_args = [(oid, email, password, str(data_dir), years, dynamic_years, i * 5.0) for i, oid in enumerate(missing_matches)]
        for result in pool.imap_unordered(process_opponent_worker, worker_args):
            if result is not None:
                second_ids, processed_id = result
                if second_ids is not None and processed_id is not None:
                    second_degree_opponents.update(second_ids)
...
def collect_second_degree_opponents(data_dir, email, password, second_degree_opponents, processed_players):
    ...
    with Pool(processes=num_processes) as pool:
        worker_args = [(oid, email, password, str(data_dir), i * 5.0) for i, oid in enumerate(second_tasks)]
...
async def complete_processing():
    ...
    success, second_degree_opponents = await collect_top_players_data(data_dir, email, password, years, True)
    if not success:
