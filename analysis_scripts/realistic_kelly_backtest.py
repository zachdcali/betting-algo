#!/usr/bin/env python3
"""
Realistic Kelly backtest with odds accuracy and conservative EV thresholds
"""
import pandas as pd
import numpy as np
import sys
...
print("="*80)
print("REALISTIC KELLY CRITERION BACKTEST")
print("="*80)

# Load and prepare 100% verified data (same as before)
...
print("\n2. Calculating market odds accuracy...")
odds_correct = 0
odds_total = 0

for i in range(len(tennis_100_verified)):
    tennis_row = tennis_100_verified.iloc[i]
    
    avg_winner_odds = tennis_row.get('AvgW')
    avg_loser_odds = tennis_row.get('AvgL')
    
    if pd.notna(avg_winner_odds) and pd.notna(avg_loser_odds):
        avg_winner_odds = float(avg_winner_odds)
        avg_loser_odds = float(avg_loser_odds)
        
        # Favorite (lower odds) wins = correct
        if avg_winner_odds < avg_loser_odds:
            odds_correct += 1
        odds_total += 1

odds_accuracy = odds_correct / odds_total if odds_total > 0 else 0
print(f"   Market odds accuracy: {odds_accuracy:.4f} ({odds_accuracy*100:.2f}%)")
print(f"   Favorite won: {odds_correct:,} out of {odds_total:,} matches")
...
# Test different Kelly thresholds
print("\n4. Testing different Kelly thresholds...")

KELLY_THRESHOLDS = [0.01, 0.02, 0.03, 0.05]  # 1%, 2%, 3%, 5%
...
for threshold in KELLY_THRESHOLDS:
    print(f"\n--- Testing {threshold*100:.0f}% Kelly threshold ---")
    ...
    for i in range(len(ml_verified_df)):
        ...
        # Place bet only if Kelly > threshold
        bet_placed = False
        
        if kelly_p1 > threshold:
        ...
        if bet_placed and bet_amount > 0 and bankroll >= 1.0:
            total_bets += 1
            ...
            bet_history.append({
                ...
                'profit': profit,
                'bankroll': bankroll,
                'kelly_used': kelly_capped if bet_placed else 0
            })
    ...
    print(f"   Bets as % of total matches: {total_bets/len(ml_verified_df)*100:.1f}%")

# Create bankroll progression chart
print(f"\n5. Creating bankroll progression chart...")
os.makedirs('analysis_scripts/charts', exist_ok=True)

plt.figure(figsize=(12, 8))
for threshold in KELLY_THRESHOLDS:
    history = results_by_threshold[threshold]['bet_history']
    if len(history) > 0:
        bankrolls = [STARTING_BANKROLL] + [bet['bankroll'] for bet in history]
        plt.plot(range(len(bankrolls)), bankrolls, label=f'{threshold*100:.0f}% threshold', linewidth=2)

plt.axhline(y=STARTING_BANKROLL, color='black', linestyle='--', alpha=0.5, label='Starting Bankroll')
plt.title('Neural Network - Bankroll Progression by Kelly Threshold')
...
plt.yscale('log')
plt.tight_layout()
plt.savefig('analysis_scripts/charts/realistic_kelly_progression.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"   Chart saved to: analysis_scripts/charts/realistic_kelly_progression.png")

print(f"\n" + "="*80)
print(f"REALISTIC KELLY BACKTEST COMPLETE")
print(f"="*80)
print(f"Market accuracy: {odds_accuracy*100:.1f}%")
print(f"Model found profitable bets on:")
for threshold in KELLY_THRESHOLDS:
    pct = results_by_threshold[threshold]['total_bets'] / len(ml_verified_df) * 100
    print(f"  {threshold*100:.0f}% threshold: {pct:.1f}% of matches")
print(f"✅ More conservative thresholds reduce unrealistic returns")